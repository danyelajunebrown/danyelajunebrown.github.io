...
app.post('/start-stream', (req, res) => {
    const { streamKey, clientId, mimeType } = req.body;
    ...
    console.log(`Stream registered for ${clientId}: ${streamInfo.rtmpUrl} (format: ${inputFormat}, mimeType: ${mimeType})`);
...
wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (data) => {
        ...
        if (!ffmpeg && !streamInfo.ffmpeg) {
            ...
            // Use the format detected by the client (webm for Chrome/Firefox, mp4 for Safari)
            const inputFormat = streamInfo.inputFormat || 'webm';
            console.log(`Using input format: ${inputFormat}`);

            ffmpeg = spawn('ffmpeg', [
                '-f', inputFormat,        // Input format from client MIME type
